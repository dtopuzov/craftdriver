/**
 * The network feature, end to end over one fixture page
 * (`network.html?bidi=true`, which disables the page's client-side fetch
 * mocking): active control via `network.mock()`/`network.intercept()`, and
 * passive observation via `waitForRequest()`/`waitForResponse()`. Both are
 * BiDi-only and share a single browser.
 */
...
describe('Network', () => {
  ...
  describe('mocking and interception', () => {
  ...
  describe('waitForRequest and waitForResponse', () => {
    ...
    async function clickAndWaitForRequest(
      ...
      const [request] = await Promise.all([
        browser.waitForRequest(matcher),
        browser.click(selector),
      ]);
      return request;
    }
